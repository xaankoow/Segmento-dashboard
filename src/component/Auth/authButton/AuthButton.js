...
export default function AuthButton({
  ...
  const { ProcessingDelay, ImportantProcessingDelay, canRequest } = useSelector(
  ...
  return (
    <button
      ...
      className={`${
        ...
        classes.includes("btn-secondary") |
          (classes.includes("btn-delete") == true)
          ? classes
          : " btn-style " + classes
      }`}
      ...
      onClick={
        ...
        (reduxHandleClick != undefined)
          ? (e) => {
              handlerClick();
              dispatch(
                reduxHandleClick(setOnclickValue != "" ? setOnclickValue : null)
              );
            }
          : (handlerClick != undefined) & (handlerClick != "")
          ? (e) => {
              handlerClick(
                (setOnclickValue != "") & (setOnclickValue != undefined)
                  ? setOnclickValue
                  : null
              );
            }
    ...
    >
      ...
      {checkingkeyLoadingProcessLoading == true ? (
        <CircularProgress
      ...
      ) : null}
      {
        <span
          className={` flex items-center ${
            checkingkeyLoadingProcessLoading == true && "opacity-0"
          } `}
        >
