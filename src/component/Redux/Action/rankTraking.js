...
export const initWorkSpacePeriodData = ({axiosController}) => {
    return async (dispatch, getState) => {
        ...
        const state = { ...getState().rankTrakingReducer }
        ...
        const workSpaceState = { ...getState().workSpaceState }
            ...
            try {
                ...
                if (!loadingState.ProcessingDelay.includes("initWorkSpacePeriodDataService")) {
                    dispatch(addLoadingItem("initWorkSpacePeriodDataService"))
                    const {data} = await initWorkSpacePeriodDataService({ axiosController: axiosController ,period:9,workspaceUuid:workSpaceState.allWorkSpace[0]})

                    if (data.code == 200 && data.status == true) {
                        state.workSpacePeriodData = data.data;
                        await dispatch({ type: "INIT_WORK_SPACE_PERIOD_DATA", payload: state })
                    } else {
