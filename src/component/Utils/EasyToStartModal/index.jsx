...
export default function BuyPlanEasyToStartModal({ handleClose, checkClose, show }) {
  ...
  const handleLockNextStep=(toggleLock)=>{
    setLockNextStep(toggleLock);
  }
      ...
      >
        <div className='w-[907px]'>
          ...
          <div className={`${stepModal==1?"px-6":stepModal>2?"px-2":stepModal==2?" px-4":""}`}>
            ...
            <FooterBtn stepModal={stepModal} setFree={setFree} setStepModal={setStepModal} handleClose={handleClose} lockNextStep={lockNextStep}/>
          </div>
        </div>
      </Modal>
    </div>
