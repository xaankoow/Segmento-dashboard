...
import { addWorkSpace, setWebAdress } from "../../Redux/Action/workSpace";
        ...
        <Modal
          ...
          <div className=" w-[862px]">
            <header className="px-2.5 border-0 bg-[#FCFCFB] mb-2">
              ...
              <div
                className="close_suport_container"
                onClick={() => {
                  navigate(-1);
                }}>
                ...
                <InputGetWorkSpaceInfo
                  ...
                  countInput={
                    stepModal == 2
                      ? addKeyCharInput
                      : stepModal == 3
                      ? addCommercialPageInput
                      : stepModal == 4
                      ? addWebsitePageInput
                      : stepModal == 5 && addCompetitorSite
                  }
                  handleAddStateCountInput={handleAddStateCountInput}
                  handleRemoveStateCountInput={handleRemoveStateCountInput}
                />
              </div>
            </body>
            <footer className="px-5">
              {stepModal != 1 ? (
                <span
                  className={"back_ico ml-24"}
                  onClick={() => setStepModal(stepModal - 1)}></span>
              ) : (
                <div></div>
              )}
              {stepModal != 1 ? (
                <AuthButton
                  ...
                  reduxHandleClick={addWorkSpace}
                  setOnclickValue={stepModal}
                  textButton={"پایان پیکربندی"}
                />
              ) : (
                <div></div>
              )}
              {stepModal != 5 ? (
                <AuthButton
                  handlerClick={setStepModal}
                  disabled={webAdress.length != 0 ? false : true}
                  setOnclickValue={stepModal + 1}
                  textButton={
                    <Fragment>
