...
export default function SelectBox({ optionItems, title, handlechange,select }) {
  ...
  useEffect(() => {
    ...
    const handleopenSelectBox=(e)=>{
      ...
      if(e.target ==e.target.querySelector('option')){
        setOpen(false)
      }
      else setOpen(true)
    ...
    <div className="flex flex-col gap-3 ">
      ...
      <div className="rounded-lg border border-[#D9D9D9] relative">
        ...
        <img src="/img/dashboard/nav_right/down.svg" alt="down" className={open ? "rotate-180 absolute left-3 top-[18px]":"absolute left-3 top-[18px]"} />
      </div>
    </div>
  );
}