            ...
            <Tab.List class="mx-3 flex  items-center mb-5 ">
                {tabsContent.map((items,index)=>{
                    ...
                    <div className='flex flex-col justify-center items-center border-none'>
                        <Tab key={index} className={activeTab ? "activeTab border-none outline-0 inline-block px-4 mt-4 mb-1 text-blue-600 rounded-t-lg  focus:text-primary text-blue-500  hover:text-primary " : "border-none outline-0 inline-block px-4 mt-4 mb-1 text-blue-600 rounded-t-lg  focus:text-primary text-blue-500  hover:text-primary "}  onClick={()=>setActiveTab(!activeTab)}>{items.title}</Tab>
